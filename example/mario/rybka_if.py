...
#problemy
#	- suradnicove systemy
#	- kreslenie lahkych veci
#	- ak ma zralok nahanat rybu -> self.svet.ryba.pozicia 
#	- vypisovanie textov
...
class Ryba(Vec):
	...
	def krok(self):
		...
		if self.svet.stlacene[pygame.K_UP]:					#IFIFIFIFIFIFIF
			...
			self.y = min( self.y + 2, VYSKA )
		elif self.svet.stlacene[pygame.K_LEFT]:
			self.x = max( self.x - 2, 0 )
		elif self.svet.stlacene[pygame.K_RIGHT]:
			self.x = min( self.x + 2, SIRKA )
...
class Jedlo(Vec):
	...
	def krok(self):
		self.y = self.y + 3
		if self.y-self.r >= VYSKA:							#IFIFIFIFIF
			self.y = - 2*self.r
